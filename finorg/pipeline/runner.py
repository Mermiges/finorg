...
def check_prerequisites(config: PipelineConfig):
    ...
    for pkg, name in [("pymupdf", "PyMuPDF"), ("PIL", "Pillow"), ("tqdm", "tqdm"),
        ...
        try:
            ...
            console.print(f"  [NO] {name}")

    for eng, pkg in [
        ("lightonocr", "transformers"),
        ("marker", "marker"),
        ("docling", "docling.document_converter"),
    ]:
        try:
            __import__(pkg)
            console.print(f"  [OK] OCR: {eng}")
        except Exception:
    ...
    if config.parallel:
        ...
        gpus = detect_gpus()
        if gpus:
            console.print(f"  [OK] GPUs: {len(gpus)}")
            for g in gpus:
...
def run_pipeline(config: PipelineConfig, start_phase: int = 1):
    config.ensure_dirs()
    log = setup_logging(config.logs_dir, config.verbose)
    t0 = time.time()
    ...
    console.print(Panel(
        ...
        f"OCR: {config.ocr_engine} ({config.ocr_model})\n"
    ...
    try:
        ...
        console.print(Panel(
            ...
            f"Output: {config.output_dir}\nReport: {config.reports_dir / 'PIPELINE_REPORT.md'}",
            style="bold green"))
